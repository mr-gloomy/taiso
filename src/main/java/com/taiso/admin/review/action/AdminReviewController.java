...
public class AdminReviewController extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 가상주소 계산
				...
				else if(command.equals("/AdminReivewContent.adr")) {
					System.out.println(" C : /AdminReivewContent.adr 호출 ");
					System.out.println(" C : [패턴2]");
					
					// AdminReivewContentAction 객체 생성
					action = new AdminReivewContentAction();
					try {
						forward = action.execute(request, response);
					} catch (Exception e) {
						e.printStackTrace();
				...
				else if(command.equals("/AdminReviewDelete.adr")) {
					System.out.println(" C : /AdminReviewDelete.adr 호출");
